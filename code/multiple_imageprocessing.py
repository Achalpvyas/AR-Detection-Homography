       ...
       if(hierarchy[0][i][3] != -1):
            ...
            if(len(approx1)>4):
                ...
                if(len(approx2)==4):
                    ...
                    print("----------")
                    if(areaOfParent < 1000 and abs(diffarea)>100 and abs(diffarea)<1000):
                        l.append(i)
                        l.append(parentId)
                        corners.append(approx2)
...
def orientation(outertag):
    outertag = np.float32(outertag)
    outertag = outertag[:,0,:]
...
def retrieveInfo(warpedtag,inbuiltFunction = 0, id = 0):
    ...
    if(neighborHoodIsWhite(thresh,[60,56])):
        ...
        if(inbuiltFunction == 1):
            cv2.imshow("Uses Opencv wrap function {}".format(id),thresh)
        else:
            cv2.imshow("Uses Custom Wrap function {}".format(id),thresh)
    ...
    for i in range(minPt[0],maxPt[0]+1):
        for j in range(minPt[1],maxPt[1]+1):
            ...
            if(dc is not None):
                ...
                if(f is not None):
                    if(hi>=0 and hi < f.shape[0] and hj>=0 and hj<f.shape[1]): 
                        f[hi,hj] =frame[i,j]
